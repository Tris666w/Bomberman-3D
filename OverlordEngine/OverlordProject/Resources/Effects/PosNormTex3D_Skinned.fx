...
VS_OUTPUT VS(VS_INPUT input){
	...
	{
		int boneIndex = input.blendIndices[index];
		if (boneIndex > -1)
		{
			transformedPosition += input.blendWeights[index] * mul(float4(input.pos, 1.0f), gBones[boneIndex]);
			transformedNormal += input.blendWeights[index] * mul(input.normal, (float3x3)gBones[boneIndex]);

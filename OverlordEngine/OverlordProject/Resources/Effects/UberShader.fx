...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	float diffuseLightValue = saturate(dot(-normal,gLightDirection));
	diffColor = saturate(gTextureDiffuse.Sample(gTextureSampler, texCoord) * diffuseLightValue);
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	//Fresnel
	float dotp = 1.f - saturate(abs(dot(viewDirection, normal)));
	float fresnel = (pow(dotp, gFresnelPower)) * gFresnelMultiplier;
	
	//Fresnel mask
	dotp = 1 - saturate(dot(float3(0, -1, 0), normal));
	float fresnelMask = pow(dotp, gFresnelHardness);
	...
	{
		finalColor = fresnel * fresnelMask * environmentColor;
	}else
	{
		finalColor = fresnel * fresnelMask * gColorFresnel;
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 reflectColor = gCubeEnvironment.Sample(gTextureSampler, reflectVec);
	reflectColor *= gReflectionStrength;

	float3 refractVec = refract(viewDirection, normal, gRefractionIndex);
	float3 refractColor = gCubeEnvironment.Sample(gTextureSampler, reflectVec);
	refractColor *= gRefractionStrength;
	
	float3 color = reflectColor + refractColor;
	return color;
...
float CalculateOpacity(float2 texCoord)
{
	...
	float4 opacity = gTextureOpacity.Sample(gTextureSampler,texCoord);
	
	return opacity.w * gOpacityIntensity;
}

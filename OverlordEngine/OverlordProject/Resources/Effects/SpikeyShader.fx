...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal, float2 texCoord)
{
	...
	temp.Position = mul(float4(pos, 1.f), m_MatrixWorldViewProj);
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	float3 basePoint,basePointNormal, top, left, right, spikeNormal;
	...
	basePointNormal = (vertices[0].Normal + vertices[1].Normal + vertices[2].Normal) / 3.f;
